...
class RestoreSessionUseCaseTest {
    ...
    @Test
    fun `returns NotAuthenticated when store is empty`() =
        runTest {
            val repo = FakeRepo()
    ...
    @Test
    fun `returns NotAuthenticated and clears when session is expired`() =
        runTest {
            val last = 1_000L
    ...
    @Test
    fun `returns NotAuthenticated and clears when uid mismatch`() =
        runTest {
            val now = 1_000L
    ...
    @Test
    fun `returns Authenticated when session valid and uid matches`() =
        runTest {
            val now = 1_000_000L
            val last = now - 5_000L
            val repo = FakeRepo(uid = "u1")
    ...
    @Test
    fun `returns NotAuthenticated and clears when currentUid is null (firebase signed out)`() =
        runTest {
            val now = 1_000_000L
            val last = now - 5_000L
