...
class RequestPasswordResetUseCaseTest {
    ...
    @Test
    fun `returns InvalidEmail without calling repo when email is malformed`() =
        runTest {
    ...
    @Test
    fun `returns Sent when repo succeeds`() =
        runTest {
            val repo = FakeAuthRepo()
            val useCase = RequestPasswordResetUseCase(repo, NoOpLogger())
    ...
    @Test
    fun `returns NetworkError when repo throws network exception`() =
        runTest {
            val repo =
                FakeAuthRepo(
    ...
    @Test
    fun `returns UnknownError for other failures`() =
        runTest {
